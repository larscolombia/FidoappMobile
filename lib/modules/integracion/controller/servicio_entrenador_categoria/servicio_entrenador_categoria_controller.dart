...
class ServiceEntrenadorController extends GetxController {
  ...
  var serviceDurations = <ServiceDuration>[].obs;
  ...
  void fetchDuration() async {
    try {
      ...
      // Reemplaza con tu URL de API real
      final response = await http.get(
        Uri.parse('${BASE_URL}duration-list-all'),
        headers: {
          'Authorization': 'Bearer ${AuthServiceApis.dataCurrentUser.apiToken}',
          'Content-Type': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        var jsonResponse = json.decode(response.body);
        print('Respuesta JSON servicios: $jsonResponse');
        var fetchedServices = (jsonResponse['data'] as List).map((duration) => ServiceDuration.fromJson(duration)).toList();
        serviceDurations.value = fetchedServices;
      } else {
        print('Error: ${response.statusCode}');
      }
    } catch (e) {
